...
#Sensitivity analysis: Remove WISC suppliment
```{r}
  ...
  for (sex_level in c("all", "Female", "Male")) {
    ...
    for (cohort_level in c("all", "home", "mirec")) {
      ...
      for(outcome in outcomes){
        ...
        boot <- qgcomp.boot(formula, expnms = mixture, data = cohort_data, family= gaussian(), q=q, B = 400, seed = 2022)
...
qgcomp_func(outcomes = c("wppsi_fsiq", "wppsi_viq", "wppsi_piq"), 
            ...
            log_BCEtP+log_BDCPP+log_DBuP+log_DPhP+log_TCS+log_BPA+log_MBP+
...
#for NCS chemicals - 15 chemicals
```{r}
qgcomp_func(outcomes = c("wppsi_fsiq", "wppsi_viq", "wppsi_piq"), 
            output_folder = "E:/BBK17/pj/data_2023apr/results/qgcomp/qgcomp_ncs_wo_wisc", 
            include_sex = TRUE, include_cohort = TRUE, 
            chemicals = "~log_Pb+log_PBDE_47+log_PCB_138+log_PCB_153+
            log_PFHxS+log_BDCPP+log_DPhP+log_TCS+log_BPA+log_MBP+log_MBZP+
            log_MCPP+log_MEP+log_di_Ethyl_OP+log_P_PB+",
            covariates= "race_bin + log_cotinine +mom_edu_cat + home_score_total + parity_n + mom_age",
            q=4,
            dat = dat)
```
...
#for PCS chemicals - 14 chemicals
```{r}
qgcomp_func(outcomes = c("wppsi_fsiq", "wppsi_viq", "wppsi_piq"), 
            output_folder = "E:/BBK17/pj/data_2023apr/results/qgcomp/qgcomp_pcs_wo_wisc", 
            include_sex = TRUE, include_cohort = TRUE, 
            chemicals = "~log_Hg+log_DMA+log_DDE+log_PCB_118+log_PCB_180+
            log_PFOA+log_PFOS+log_BCEtP+log_DBuP+log_sigma_DEHP+log_MIBP+
            log_di_Methyl_OP+log_B_PB+log_M_PB+",
            covariates= "race_bin + log_cotinine +mom_edu_cat + home_score_total + parity_n + mom_age",
            q=4,
            dat = dat)
```
...
```{r}
#29-chem: E:/BBK17/pj/data_2023apr/results/qgcomp/gcomp_29chem
#NCS: E:/BBK17/pj/data_2023apr/results/qgcomp/qgcomp_ncs
#PCS: E:/BBK17/pj/data_2023apr/results/qgcomp/qgcomp_pcs

results<- get_gcomp_estimates(results_folder = "E:/BBK17/pj/data_2023apr/results/qgcomp/qgcomp_pcs_wo_wisc")

write_csv(results, "E:/BBK17/pj/data_2023apr/results/qgcomp/qgcomp_pcs_wo_wisc/r_obj_summ.csv")
```
...
ggsave("E:/BBK17/pj/data_2023apr/results/qgcomp/qgcomp_pcs_wo_wisc/qgcomp_pcs_wo_wisc.tiff", 
       width = 8.5,height = 6,
       dpi=300)
```
...
qgcomp_weights <- extract_weights("E:/BBK17/pj/data_2023apr/results/qgcomp/qgcomp_pcs_wo_wisc")
write_csv(qgcomp_weights, "E:/BBK17/pj/data_2023apr/results/qgcomp/qgcomp_pcs_wo_wisc/chem_weight.csv")
```
...
ggplot(dat, aes( y = chemical, x=cohort, fill = weight))+
  ...
  scale_fill_gradient2(low = "red", mid = "gray", high = "blue", 
                       midpoint = 0, name = "Chemical biomarker weight",
...
#export figure
```{r}
ggsave("E:/BBK17/pj/data_2023apr/results/qgcomp/qgcomp_pcs_wo_wisc/gcomp_pcs_wo_wisc_weight.tiff", 
       width = 11,height = 6,
       dpi=300)
```
