...
for group lasso assist - 15 chemicals selected by both group lasso models
```{r}
dat <- dat |>
...
for LASSO assisted - all outcomes - 9 chemicals
```{r}
dat <- dat |>
  select(c(5,7,14,18:21,23,29:41)) |>
  drop_na()|>
...
#group lasso assist (commonly picked by grpreg and gglasso): log_Pb+log_PBDE_47+log_PCB_138+log_PCB_153+log_PFHxS+log_BDCPP+log_DPhP+log_TCS+log_BPA+log_MBP+log_MBZP+log_MCPP+log_MEP+log_di_Ethyl_OP+log_P_PB
  ...
  for (sex_level in c("all", "Female", "Male")) {
    ...
    for (cohort_level in c("all", "home", "mirec")) {
      ...
      for(outcome in outcomes){
        formula <- as.formula(paste(outcome, "~ log_Pb+log_PBDE_47+log_PCB_138+log_PCB_153+log_PFHxS+log_BDCPP+log_DPhP+log_TCS+log_BPA+log_MBP+log_MBZP+log_MCPP+log_MEP+log_di_Ethyl_OP+log_P_PB + ", sex_formula, cohort_formula,"race_bin + log_cotinine +mom_edu_cat + home_score_total + parity_n + mom_age"))
...
qgcomp_func(outcomes = c("wppsi_fsiq", "wppsi_viq", "wppsi_piq"), 
            folder_path = "E:/BBK17/pj/data_2023apr/results/qgcomp/lasso_15chem", 
...
folder_location <- "E:/BBK17/pj/data_2023apr/results/qgcomp/lasso_15chem"
...
write_csv(results, "E:/BBK17/pj/data_2023apr/results/qgcomp/lasso_15chem/r_obj_summ.csv")
```
...
ggsave("E:/BBK17/pj/data_2023apr/results/qgcomp/lasso_15chem/lasso_15chem_plot.tiff", 
       width = 8.5,height = 6,
       dpi=300)
```
