...
dat_test <- dat |>
  filter(cohort=="2")|>
  select(4,5,6,7,17,18,19,21,20,22,23,24,25,26,28:29,30:32, 35:41) |>
  mutate_at(vars(1:16), log10) |>
  rename_with(~ paste0("log_", .x), 1:16)


mixture<- names(dat_test[c(1:16)])

nb_gcomp_fit<- qgcomp.noboot(wppsi_fsiq ~ .,
                             expnms = mixture,
                             data = dat_test[-c(18,19)],
                             family= gaussian(), q=10)
...
sensitivity plot
```{r}
res<- read_csv("E:/BBK17/pj/wppsi_results/qgcomp_prog_rep.csv")
...
ggplot(res, aes(x = as.factor(model), y = beta, ymin = ll, ymax = ul)) + 
  ...
  labs(x=NULL,
       ...
       caption = "Mixture includes 16 environmental chemicals: Triclosan, BPA, POPs (DDE,PBDE-47, PCB-118, PCB-138), Parabens (Methyl-paraben, Propyl-paraben), Phthalates (MBP, MBZP, MCPP, MEP, MIBP, Sigma-DEHP), OP-pesticides (Sigma-di-ethyl-op, Sigma-di-methyl-op)
...
ggsave("E:/BBK17/pj/wppsi_results/qgcomp_prog_rep.tiff", 
       width = 12,height = 8,
       dpi=300)
