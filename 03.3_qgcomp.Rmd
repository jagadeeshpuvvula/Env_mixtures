...
```{r}
qgcomp_weights <- extract_weights("E:/BBK17/pj/data_2023apr/results/qgcomp/lasso_pos_14chem")
write_csv(qgcomp_weights, "E:/BBK17/pj/data_2023apr/results/qgcomp/lasso_pos_14chem/chem_weight.csv")
```
...
ggplot(dat, aes(x = outcome, y = chemical, fill = weight))+
  geom_tile()+
  geom_text(aes(label = round(weight, 3)), 
            color = "black", size = 3, fontface = "bold", show.legend = FALSE)+
  labs(x = "", y = "", fill = "weight")+
  facet_grid(.~gender*cohort, scales = "free_y", space = "free_y", switch = "both")+
  theme_bw()+
