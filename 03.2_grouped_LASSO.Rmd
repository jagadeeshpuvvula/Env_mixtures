...
v.group<- as.integer(c(rep(1,times=3), rep(2,times=6), rep(3, times=3),
...
#cv
set.seed(5678)
la_cv<-cv.glmnet(x=X, y=y_fsiq, family="gaussian", alpha=1, nfolds=5)
paste(la_cv$lambda.min, la_cv$lambda.1se)

# group lasso with gglasso
set.seed(5678)
gr_cv<-cv.gglasso(x=X, y=y_fsiq, group=v.group, loss="ls", pred.loss="L2",  nfolds=5)
paste(gr_cv$lambda.min, gr_cv$lambda.1se)

# group lasso with grpreg
grpp_cv<-cv.grpreg(X, y_fsiq, group=v.group, penalty="grLasso",seed=5678,nfolds = 5)
grpp_cv$lambda.min


# extract minimum lambda values for each model
la_min_lambda <- la_cv$lambda.min
gr_min_lambda <- gr_cv$lambda.min
grpp_min_lambda <- grpp_cv$lambda.min

# extract corresponding beta values
la_min_beta <- coef(la_cv, s = la_min_lambda)[-1]
gr_min_beta <- coef(gr_cv, s = gr_min_lambda)[-1]
grpp_min_beta <- coef(grpp_cv, lambda = grpp_min_lambda)[-1]


# create data frame
df_min_lambda <- data.frame(variable = names(grpp_min_beta),
                            group = v.group,
                            lasso = la_min_beta,
                            gr_lasso = gr_min_beta,
                            grpreg_lasso = grpp_min_beta)
...
```{r}
set.seed(5678)
la_cv.1<-cv.glmnet(x=X, y=y_fsiq, family="gaussian", alpha=1,nfolds=5)

# group lasso with gglasso
set.seed(5678)
gr_cv.1<-cv.gglasso(x=X, y=y_fsiq, group=1:ncol(X), loss="ls", pred.loss="L2",  nfolds=5)

# group lasso with grpreg
grpp_cv.1<-cv.grpreg(X, y_fsiq, group=1:ncol(X), penalty="grLasso",seed=5678,nfolds = 5)


# extract minimum lambda values for each model
la_min_lambda.1 <- la_cv.1$lambda.min
gr_min_lambda.1 <- gr_cv.1$lambda.min
grpp_min_lambda.1 <- grpp_cv.1$lambda.min

# extract corresponding beta values
la_min_beta.1 <- coef(la_cv.1, s = la_min_lambda)[-1]
gr_min_beta.1 <- coef(gr_cv.1, s = gr_min_lambda)[-1]
grpp_min_beta.1 <- coef(grpp_cv.1, lambda = grpp_min_lambda)[-1]


# create data frame
df_min_lambda.1 <- data.frame(variable = names(grpp_min_beta.1),
                            group.1 = v.group,
                            lasso.1 = la_min_beta.1,
                            gr_lasso.1 = gr_min_beta.1,
                            grpreg_lasso.1 = grpp_min_beta.1)



summary_fin<- cbind(df_min_lambda, df_min_lambda.1[3:5])
```

#visualize both cross validated lambda min and lasso regressions as one group
```{r}
library(reshape2)

# Melt the data to long format and rename the value column
melted <- melt(summary_fin, id.vars = c("variable", "group"), variable.name = "type", value.name = "value")

# Define color palette
colors <- c("#FF0000", "#FFFFFF", "#00FF00")

# Create the heatmap
ggplot(data = melted, aes(x = variable, y = interaction(variable, group), fill = value)) +
  geom_tile(color = "white") +
  scale_fill_gradientn(colors = colors) +
  labs(title = "Heatmap of Summary_fin Data") +
  xlab("Variable") +
  ylab("Group") +
  theme_minimal()
