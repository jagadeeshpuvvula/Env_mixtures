...
race<- baseq |> 
  select(c(2,10:15)) |>
  mutate(aborignal = replace_na(aborignal, 0))

race$screen <- rowSums(race[c(2:7)])
race$race_cat<- apply(race[-1], 1, function(i) names(race[-1])[which(i == 1)])
dat<- race[c(1,8)]
remove(race)
...
```{r}
dat<- read_csv("E:/BBK17/4. BBK17_CD+ CRF data corrections_Nov18 2022/CDPLUS.2_NEURO.1_20221117/HOME_SCORES_Yes to NEURO BBK.csv") |>
  clean_names() |>
