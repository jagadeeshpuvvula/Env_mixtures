...
var_groups <- c(POPs = "Group A", Parbens = "Group B", TCS_BPA = "Group C",
                 Phthalates = "Group D", OP_pest. = "Group E", BPA = "Group F",
                 Phthalate = "Group G", OP_Pest. = "Group H")


dat <- dat |>
  select(4,5,17,18,19,21,20,22,23,24,25,26,28:29)

group<- as.factor(c(rep("POPs",times=2), rep("Parabens",times=2), 
                    rep("TCS_BPA",times=2), rep("Phthalates",times=6), 
                    rep("OP_Pest.",times=2)))


cormat <- round(x=cor(dat, method = "spearman", use = "complete.obs"), digits = 2) %>%
  melt() %>%
  clean_names() %>%
  mutate(group = var1 %>% factor(levels = var_groups))

ggplot(cormat, aes(x = var2, y = var1, fill = value, label = value, group = group)) +
  geom_tile(color = "white") +
  ...
  geom_vline(xintercept = c(2.5, 4.5, 10.5, 12.5, 14.5), color = "black") +
  geom_hline(yintercept = c(2.5, 4.5, 10.5, 12.5, 14.5), color = "black") 
