...
dat<- dat |>
  ...
  rename(Pb=pb, Hg=hg, DMA=dma, DDE= dde, PBDE_47 = pbde47,
         ...
         mom_edu_cat=edu3)|>
  mutate(race = ifelse(race == "white", 1, 2))|>
  select(c(Pb, Hg, DMA, DDE, PBDE_47, PCB_118, PCB_138, PCB_153, PCB_180,
          ...
          cohort, city, sex, race,  mom_edu_cat, parity_n, 
...
bkmr_cohort <- function(data, folder_path, include_sex = TRUE, include_cohort = TRUE,
  ...
  for (cohort_level in c("all", "home", "mirec")) {
    ...
    for(i in 1:ncol(outcome)) {
      ...
      knots100 <- fields::cover.design(ln_mixture, nd = round(0.015 * nrow(ln_mixture)))$design
      
      bkmr <- kmbayes(y = outcome[,i], Z = ln_mixture, X = covariates, iter = 100, 
...
bkmr_sex(data=dat, 
          ...
          ln_mixture_vars= c("Pb","Hg", "DMA","DDE","PBDE_47","PCB_118","PCB_138",
                             "PCB_153","PCB_180","PFHxS", "PFOA","PFOS","BCEtP","BDCPP",
                             "DBuP","DPhP","TCS","BPA","MBP","MBZP","MCPP",
                             "sigma_DEHP", "MEP","MIBP","di_Ethyl_OP", "di_Methyl_OP",
                             "B_PB","M_PB", "P_PB"),
          groups=c(1, 1, 1, 2, 2, 2, 2, 2, 2, 3, 3, 3, 4, 4, 4, 4, 5, 5, 6, 6, 
                   6, 6, 6, 6,7, 7, 8, 8, 8),
...
```{r}
results <- extract_data_from_folder("E:/BBK17/pj/data_2023apr/results/bkmr")
```
...
ggplot(overall_risk, aes(overall_risk_df.quantile, overall_risk_df.est, 
                         ymin = overall_risk_df.est - 1.96*overall_risk_df.sd, 
                         ymax = overall_risk_df.est + 1.96*overall_risk_df.sd)) + 
    geom_hline(yintercept = 00, linetype = "dashed", color = "gray") + 
    geom_pointrange() + theme_minimal() +
    labs(x = "Quantile", y = "Estimate")+
    facet_grid(.~factor(overall_risk_df.measure_name, levels = c("fsiq", "piq", "viq")))+
