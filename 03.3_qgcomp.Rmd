...
Select number of chemicals here
```{r}
  ...
  for (sex_level in c("all", "Female", "Male")) {
    ...
    for (cohort_level in c("all", "home", "mirec")) {
      ...
      for(outcome in outcomes){
        formula <- as.formula(paste(outcome, "~ log_Pb+log_Hg+log_DMA+log_DDE+log_PBDE_47+log_PCB_118+log_PCB_138+log_PCB_153+log_PCB_180+log_PFHxS+log_PFOA+log_PFOS+log_BCEtP+log_BDCPP+log_DBuP+log_DPhP+log_TCS+log_BPA+log_MBP+log_MBZP+log_MCPP+log_sigma_DEHP+log_MEP+log_MIBP+log_di_Ethyl_OP+log_di_Methyl_OP+log_B_PB+log_M_PB+log_P_PB + ", sex_formula, cohort_formula,"race_bin + log_cotinine +mom_edu_cat + home_score_total + parity_n + mom_age"))
...
qgcomp_func(outcomes = c("wppsi_fsiq", "wppsi_viq", "wppsi_piq"), 
            folder_path = "E:/BBK17/pj/data_2023apr/results/qgcomp/29chem", 
...
folder_location <- "E:/BBK17/pj/data_2023apr/results/qgcomp/29chem"
...
write_csv(results, "E:/BBK17/pj/data_2023apr/results/qgcomp/29chem/r_obj_summ.csv")
```
...
ggplot(res, aes(x = as.factor(outcome), y = estimate, ymin = lower_ci, ymax = upper_ci)) + 
  ...
  labs(x=NULL,
       ...
       Effect estimates adjusted for cohort, city (for pooled and MIREC), Maternal: serum cotinine, education, race, age, parity; Child: gender, HOME-score
...
ggsave("E:/BBK17/pj/data_2023apr/results/qgcomp/29chem/29_chem_plot.tiff", 
       width = 8.5,height = 6,
       dpi=300)
```
