...
```{r}
lm_func <- function(dependent_vars, independent_vars, covariates, data, include_sex = TRUE, include_cohort = TRUE) {
...
ggplot(dat_lm_res, aes(x = outcome  , y = chemical, fill = coefficient))+
  geom_tile()+
  ...
  facet_grid(.~sex_level*cohort_level, scales = "free_y", space = "free_y", switch = "both")+
  theme_bw()+
